  ...
  switch (rally.phase) {
    case 'serve': {
      if (result === 'error') {
        ...
        if (teamId !== homeTeamId) {
          // Opponent ace → don't score yet, go to reception so user can pick who failed
          mainEvent.scoreHome = scoreHome
          mainEvent.scoreAway = scoreAway
          newEvents.push(mainEvent)
          newPhase = 'reception'
        } else {
          // Home ace → +1 home, rally over (auto-generates reception error for opponent)
          scoreChange = getScoreChange(action, result, teamId, homeTeamId)
          mainEvent.scoreHome = scoreHome + (scoreChange?.home || 0)
          mainEvent.scoreAway = scoreAway + (scoreChange?.away || 0)
          newEvents.push(mainEvent)
        ...
        // Neutral serve (in play, pressure)
        mainEvent.scoreHome = scoreHome
        mainEvent.scoreAway = scoreAway
        newEvents.push(mainEvent)
        newPhase = 'reception'
    ...
    case 'reception': {
      if (result === 'error') {
        ...
        // Auto-create ace event for server ONLY if no real ace was already recorded
        const hasRealAce = rally.events.some(
          e => e.action === 'serve' && e.result === 'success' && !e.meta?.autoGenerated,
        )
        if (!hasRealAce) {
          const autoAceEvent: MatchEvent = {
            id: generateId(),
            matchId: match.id,
            rallyId: rally.rallyId,
            setNumber: match.currentSet,
            timestamp: Date.now() + 1,
            teamId: rally.servingTeamId,
            playerId: rally.serverPlayerId,
            action: 'serve',
            result: 'success',
            scoreHome: mainEvent.scoreHome,
            scoreAway: mainEvent.scoreAway,
            meta: { quality: 'ace', autoGenerated: true },
          }
          newEvents.push(autoAceEvent)
        }

        newPhase = 'rally_over'
        rallyComplete = true
        autoReceptionError = true
      } else {
        ...
        sections: [
          {
            ...
            actions: [
              ...
              { action: 'attack', result: 'error', quality: 'attack_error', label: '−', variant: 'error' },
              { action: 'attack', result: 'error', quality: 'attack_blocked', label: 'Б', variant: 'error' },
              { action: 'attack', result: 'neutral', label: '0', variant: 'neutral' },
            ],
          },
          {
            title: 'Блок',
...
export function buildRallyRecord(
  ...
  const wasAce = allEvents.some((e) => e.action === 'serve' && e.result === 'success' && !e.meta?.autoGenerated)
    || allEvents.some((e) => e.action === 'reception' && e.result === 'error')
